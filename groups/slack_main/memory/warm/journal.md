...
**Goal:** Implement JOTF Skill Manifest Standard — rename all skills to `jotf.{domain}.{function}` namespace
**Actions:**
1. Both founders approved the manifest standard and rename plan
2. Cloned `SirSerendip/nanoclaw-jedi-style` to writable location (project mount is EROFS)
3. Renamed all 13 container skill directories to dot-notation namespace
4. Updated every SKILL.md with `domain`, `version: 0.1.0`, and `inputs`/`outputs` where applicable
5. Updated `container-runner.ts` comments for new skill names
6. Created PR #1: https://github.com/SirSerendip/nanoclaw-jedi-style/pull/1
**Decisions:**
- CLI script names unchanged (ftp-upload, smtp-send etc.) — zero UX disruption
- `allowed-tools` patterns unchanged — still reference the bash script names, not skill names
- No groups use `allowedSkills` filter, so directory renames are safe
- Manifest standard doc saved at `/workspace/group/JOTF-SKILL-MANIFEST.md`
**Outcome:** PR created and posted to #la-force. Awaiting merge.
**Lessons:** `/workspace/project/` is read-only inside containers. Must clone repo to `/tmp/` for changes, push via gh CLI.
**Next:** After PR merge — test invocation, then build `jotf.taster.persona` as first new pipeline skill.
